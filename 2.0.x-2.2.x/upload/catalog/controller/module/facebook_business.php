...
class ControllerModuleFacebookBusiness extends Controller {
    ...
    public function appStorePing() {
        ...
        $this->load->model('module/facebook_business');
        $json['version'] = $this->model_module_facebook_business->getPluginVersion();
        $json['store_url'] = $this->url->link('module/facebook_business', '', true);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($signature == 'f35ca30bfe0b48aa63404483bef6697e94b3f732') {
                ...
                if (isset($this->request->post['facebook_system_user_access_token'])) {
                    $this->model_module_facebook_business->installFBE($this->request->post);
                } else {
                    $this->model_module_facebook_business->uninstallFBE();
                }
    ...
    private function estimateFeedGenerationTimeWithDecay($feed_gen_time) {
        ...
        $this->model_module_facebook_business->updateFacebookSettings($data);
    }
    ...
    private function formatProductDetails($product_info) {
        $special_info = $this->model_module_facebook_business->getProductSpecials($product_info['product_id']);
    ...
    private function getCategory($product_info) {
        $product_to_facebook = $this->model_module_facebook_business->getProductToFacebook($product_info['product_id']);
    ...
    private function getCondition($product_id) {
        $product_to_facebook = $this->model_module_facebook_business->getProductToFacebook($product_id);
